	...
	try :
		...
		for field in desc.Fields:
			...
			if (field.domain is not None and field.domain != ""):
				...
				gdbPath += ".gdb"
				arcpy.DomainToTable_management(gdbPath, field.domain, "in_memory/" + \
					field.domain, CODE_FIELD_NAME, DESCRIPTION_FIELD_NAME)
		...
		for feature in features : 
			...
			if not ((messageTypeField == '') or (messageTypeField is None)):
				try :   
					messageType = feature.getValue(messageTypeField)           
				except :             
